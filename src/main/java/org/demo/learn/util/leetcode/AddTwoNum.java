...
import org.demo.learn.util.leetcode.linkedList.ArrayToLinkedList;
import org.demo.learn.util.leetcode.linkedList.SingleListNode;
...
public class AddTwoNum {
    ...
    public static void main(String[] args){
        ...
        SingleListNode twoNum = recursiveAddTwoNum(nodes1, nodes2);
        SingleListNode.print(twoNum);
    }

    public static SingleListNode recursiveAddTwoNum(SingleListNode node1, SingleListNode node2) {
        // 只有在node1和node2有一个不为空或者有进位的情况才需要继续
        if (node1 != null || node2 != null || carry == 1) {
            // 如果短链表循环结束，且没有进位，返回长链表到上一栈
            if (node1 == null && node2 != null && carry == 0) {
                return node2;
            } else if (node2 == null && node1 != null && carry == 0) {
                return node1;
            }
            ...
            // 建立空指向的节点
            SingleListNode node = new SingleListNode(newValue, null);
            // 当前节点的下一个节点应该从递归中获取
            node.next = recursiveAddTwoNum(node1, node2);
            return node;
        }
        return null;
    }
